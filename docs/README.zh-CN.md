...
在 **设置 → 订阅 → Jev 自动分类** 中，每个用户可以保存自己的 TypeSafe API key 并开启功能，默认关闭。新增订阅时，输入暂停后会从该用户已有分类中自动选中置信度足够的建议。手动选择或清空分类后不再自动修改；编辑已有订阅不会触发分类。建议不可用或不确定时，不影响保存。

只向 TypeSafe 发送订阅名称、网址域名和分类名称，不发送备注、金额、网址路径或查询参数。密钥在服务器加密保存，不通过 API 返回，也不包含在用户 JSON 导出中。完整数据库备份应同时保管服务器的设置加密密钥，与其他加密凭据一致。移除 Jev 密钥会同时关闭功能。

同一设置区域可以在不发送订阅数据的情况下测试已保存的凭据，并显示最新连接状态、最近成功连接时间，以及当前密钥对应的分类请求数和建议数。录入订阅时分类失败仍会静默降级；凭据、额度、服务可用性和接口兼容性信息只在设置页显示。

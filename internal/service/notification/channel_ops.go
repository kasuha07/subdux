...
func (s *Service) ListChannels(userID uint) ([]model.NotificationChannel, error) {
...
func (s *Service) UpdateChannel(userID, channelID uint, input UpdateChannelInput) (*model.NotificationChannel, error) {
...
func (s *Service) ensureEnabledChannelLimit(userID uint) error {
...
func (s *Service) TestChannel(userID, channelID uint) error {
	var channel model.NotificationChannel
	if err := s.DB.Where("id = ? AND user_id = ?", channelID, userID).First(&channel).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return errors.New("channel not found")
	...
	templateData := s.buildTemplateData(subscriptionservice.ReminderCandidateFromPreviewSubscription(*testSubscription, testBillingDate, 3), &user)

...
func (s *Service) EnqueuePendingNotifications() error {
...
func (s *Service) processUserNotifications(userID uint) error {
	...
	candidates, err := s.subscriptionReminderProvider().ListReminderCandidates(context.Background(), subscriptionservice.ListReminderCandidatesInput{
		UserID: userID,
		Now:    now,
		Policy: reminderPolicyFromModel(policy),
	})
	if err != nil {
		return err
	}
...
func (s *Service) manualRenewEndedNotificationAlreadySent(subscriptionID uint, endedAt time.Time) (bool, error) {
	var count int64

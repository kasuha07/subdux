...
func (s *Service) enqueueNotificationOutbox(job notificationOutboxJob) error {
...
func (s *Service) notificationAlreadySent(subscriptionID uint, channelType, triggerType string, notifyDate, originalNotifyDate, dedupeDate time.Time) (bool, error) {
	var count int64
...
func (s *Service) DispatchDueNotificationOutbox(ctx context.Context) (*NotificationDispatchSummary, error) {
	if ctx == nil {
...
func (s *Service) expireNotificationOutbox() (int, error) {
	now := pkg.NowUTC()
...
func (s *Service) claimDueNotificationOutbox(ctx context.Context, batchSize int, leaseTTL time.Duration) ([]model.NotificationOutbox, error) {
	if batchSize <= 0 {
		return nil, nil
	}
...
func (s *Service) cancelOutboxIfNoLongerDeliverable(job model.NotificationOutbox) string {
	...
	reminderPolicy := subscriptionservice.ReminderPolicy{}
	if job.TriggerType != notificationTriggerManualEnded {
		...
		reminderPolicy = reminderPolicyFromModel(policy)
	}

	matches, reason, err := s.subscriptionReminderProvider().ValidateNotificationOutboxJob(context.Background(), subscriptionservice.ValidateNotificationOutboxJobInput{
		Job:    job,
...
func (s *Service) loadOutboxChannel(job model.NotificationOutbox) (*model.NotificationChannel, string) {
...
func (s *Service) markNotificationOutboxSent(job model.NotificationOutbox) error {
	now := pkg.NowUTC()
...
func (s *Service) markNotificationOutboxFailed(job model.NotificationOutbox, sendErr error) error {
	now := pkg.NowUTC()
...
func (s *Service) releaseNotificationOutboxForRetry(job model.NotificationOutbox, err error) error {
	now := pkg.NowUTC()
	return s.DB.Model(&model.NotificationOutbox{}).
		Where("id = ? AND locked_by = ? AND status = ?", job.ID, s.notificationOwnerID(), notificationOutboxStatusProcessing).
		Updates(map[string]interface{}{

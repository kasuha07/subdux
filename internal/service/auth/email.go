...
var (
	ErrRegistrationDisabled                  = serviceerr.New(serviceerr.KindForbidden, "registration_is_disabled", "registration is disabled")
	ErrRegistrationEmailVerificationDisabled = serviceerr.New(serviceerr.KindInvalid, "registration_email_verification_is_disabled", "registration email verification is disabled")
	ErrVerificationCodeRequired              = serviceerr.New(serviceerr.KindInvalid, "verification_code_is_required", "verification code is required")
	ErrVerificationCodeInvalid               = serviceerr.New(serviceerr.KindInvalid, "invalid_or_expired_verification_code", "invalid or expired verification code")
	ErrVerificationCodeTooManyAttempts       = serviceerr.New(serviceerr.KindInvalid, "verification_code_has_too_many_failed_attempts_request_a_new_code", "verification code has too many failed attempts, request a new code")
	ErrVerificationCodeTooFrequent           = serviceerr.New(serviceerr.KindTooMany, "please_wait_before_requesting_another_verification_code", "please wait before requesting another verification code")
	ErrInvalidEmail                          = serviceerr.New(serviceerr.KindInvalid, "invalid_email", "invalid email")
	ErrSMTPUnavailable                       = serviceerr.New(serviceerr.KindInvalid, "email_service_is_unavailable", "email service is unavailable")
	ErrEmailAlreadyRegistered                = serviceerr.New(serviceerr.KindConflict, "email_already_registered", "email already registered")
	ErrUsernameAlreadyTaken                  = serviceerr.New(serviceerr.KindConflict, "username_already_taken", "username already taken")
	ErrUserNotFound                          = serviceerr.New(serviceerr.KindNotFound, "user_not_found", "user not found")
	ErrCurrentPasswordIncorrect              = serviceerr.New(serviceerr.KindInvalid, "current_password_is_incorrect", "current password is incorrect")
	ErrNewEmailSameAsCurrent                 = serviceerr.New(serviceerr.KindInvalid, "new_email_must_be_different_from_current_email", "new email must be different from current email")
	ErrPasswordTooLong                       = serviceerr.New(serviceerr.KindInvalid, "password_must_not_exceed_72_bytes", "password must not exceed 72 bytes")
)

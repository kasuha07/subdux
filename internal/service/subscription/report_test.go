...
func TestGetAnalyticsReportAggregatesSubscriptionSpend(t *testing.T) {
	...
	videoCategory := model.Category{UserID: user.ID, Name: "Video", NameCustomized: true}
	if err := db.Create(&videoCategory).Error; err != nil {
		t.Fatalf("create category failed: %v", err)
	}

	cardMethod := model.PaymentMethod{UserID: user.ID, Name: "Card", NameCustomized: true}
	if err := db.Create(&cardMethod).Error; err != nil {

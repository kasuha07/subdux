...
func (s *Service) List(userID uint) ([]model.Subscription, error) {
...
func (s *Service) GetByID(userID, id uint) (*model.Subscription, error) {
...
func (s *Service) Create(userID uint, input CreateSubscriptionInput) (*model.Subscription, error) {
	...
	if err := s.DB.Transaction(func(tx *gorm.DB) error {
		...
		return (&Service{DB: tx}).recordSubscriptionCreated(userID, sub)
	}); err != nil {
		return nil, err
	}
...
func (s *Service) validateCategory(userID, categoryID uint) error {
...
func (s *Service) validatePaymentMethod(userID, paymentMethodID uint) error {

...
type ReminderPolicy = subscriptionReminderPolicy
...
type ReminderCandidate = subscriptionReminderCandidate
...
type ReminderTemplateSnapshot = subscriptionReminderTemplateSnapshot

type ListReminderCandidatesInput struct {
	UserID uint
	Now    time.Time
	Policy ReminderPolicy
}

type ValidateNotificationOutboxJobInput struct {
	Job    model.NotificationOutbox
	Policy ReminderPolicy
}

func (s *Service) ListReminderCandidates(ctx context.Context, input ListReminderCandidatesInput) ([]ReminderCandidate, error) {
	return listSubscriptionReminderCandidates(ctx, s.DB, input.UserID, input.Now, input.Policy)
}

func (s *Service) ListEndedManualRenewNotificationCandidates(ctx context.Context, userID uint, referenceDate time.Time) ([]ReminderCandidate, error) {
	return listEndedManualRenewNotificationCandidates(ctx, s.DB, userID, referenceDate)
}

func (s *Service) ValidateNotificationOutboxJob(ctx context.Context, input ValidateNotificationOutboxJobInput) (bool, string, error) {
	return validateSubscriptionNotificationOutboxJob(ctx, s.DB, input.Job, input.Policy)
}

func ReminderCandidateFromPreviewSubscription(sub model.Subscription, notifyDate time.Time, daysUntil int) ReminderCandidate {
	return subscriptionReminderCandidateFromPreviewSubscription(sub, notifyDate, daysUntil)
}
...
func subscriptionBillingReminderCandidates(
	...
	for _, triggerType := range triggerTypes {
		...
		if triggerType == TriggerManualDaily {
			dedupeDate = scanDate
		}
...
func subscriptionReminderTriggerTypes(daysUntilBilling, daysBefore int, notifyOnDueDay bool) []string {
	...
	if daysUntilBilling == daysBefore && daysBefore > 0 {
		triggers = append(triggers, TriggerDaysBefore)
	}
	if daysUntilBilling == 0 && notifyOnDueDay {
		triggers = append(triggers, TriggerDueDay)
	}
	return triggers
}
...
func subscriptionReminderTriggerTypesForSubscription(
	...
	if normalizeRenewalMode(renewalMode) == renewalModeManualRenew &&
		...
		triggers = append(triggers, TriggerManualDaily)
	}
	return triggers
}
...
func subscriptionOutboxMatchesCurrentBillingReminder(
	...
	daysUntilBilling := pkg.DaysUntil(*sub.NextBillingDate, systemLoc)
	if job.TriggerType == TriggerManualDaily {

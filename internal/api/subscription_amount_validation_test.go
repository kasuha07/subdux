...
func TestCreateSubscriptionRejectsOutOfRangeAmount(t *testing.T) {
	tests := []struct {
		...
		{name: "above the maximum", amount: "500000000000.01", want: "amount_too_large"},
...
func TestCreateSubscriptionAcceptsMaximumAmount(t *testing.T) {
	...
	rec := postSubscriptionJSON(t, handler, userID,
		`{"name":"Video Pro","amount":500000000000,"currency":"USD","status":"active","renewal_mode":"auto_renew","billing_type":"recurring","recurrence_type":"interval","interval_count":1,"interval_unit":"month","next_billing_date":"2026-04-15"}`)

...
- `src/lib/api.ts` 统一处理 API 请求、Bearer Token、刷新失败后的 401 跳转，以及会话缓存
...
- `@` 路径别名映射到 `src/`
- 开发模式下，`/api` 会代理到 `http://localhost:8080`
- 生产模式下，`web/dist/` 会由根项目通过 `go:embed` 嵌入 Go 二进制统一提供

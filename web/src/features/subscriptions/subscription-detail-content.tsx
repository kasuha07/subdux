...
export default function SubscriptionDetailContent({
  ...
  function formatAmount(amount: number, currency = subscription.currency ?? "USD") {
    const symbol = currency.toUpperCase() === subscription.currency.toUpperCase()
      ? currencySymbol
      : undefined
  ...
  if (!detail) {
    return <OverviewFallback subscription={subscription} />
  }
